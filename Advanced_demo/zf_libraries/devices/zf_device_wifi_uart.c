...
static  uint8_t                 wifi_uart_buffer[WIFI_UART_BUFFER_SIZE];            // 数据存放数组
...
// 参数说明     timeout         超时时间
// 返回参数     uint8_t           0：模块响应指定数据   1：模块未响应指定数据或超时
// 备注信息     内部调用
//--------------------------------------------------------------------------------------------------
    ...
    if(WIFI_UART_SERVER_ON == wifi_uart_information.wifi_uart_connect_state)
    {
        ...
        else
        {
            while(len --)
            {
                ...
                uart_write_byte(WIFI_UART_INDEX, (uint8_t*)buff);                        // 发送最后的数据
                buff ++;
            }
        }
    }
...
// 返回参数     uint16_t          返回实际接收到的数据长度
// 使用示例     uint8_t test_buffer[256]; wifi_uart_read_buffer(&test_buffer[0], sizeof(test_buffer));
// 备注信息
//-------------------------------------------------------------------------------------------------------------------
...
// 返回参数     uint8_t           模块初始化状态 0-成功 1-错误
